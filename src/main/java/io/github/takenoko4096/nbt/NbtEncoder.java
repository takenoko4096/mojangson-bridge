...
public final class NbtEncoder {
    ...
    private NbtEncoder(DataOutputStream stream, String name, MojangsonCompound compound) {
        this.stream = stream;
        this.name = name;
    ...
    public static byte[] compress(MojangsonCompound compound) {
        ...
        try (final DataOutputStream stream2 = new DataOutputStream(new GZIPOutputStream(stream1))) {
            final var writer = new NbtEncoder(stream2, new String(new char[0]), compound);
            writer.write();
        }
        catch (IOException e) {
            throw new NbtWriteException(e);
